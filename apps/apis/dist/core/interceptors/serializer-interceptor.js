...
let SerializerInterceptor = class SerializerInterceptor {
    intercept(context, next) {
        return next.handle().pipe((0, rxjs_1.map)((data) => {
            ...
            let meta = new dtos_1.MetaResponseDto(statusCode);
            if (data?.pagination) {
                meta = new dtos_1.MetaResponseDto(statusCode, undefined, undefined, data.pagination);
                data = data.result;
            }
            return new dtos_1.ResponseDto(data, meta);
        }));
    }
};
